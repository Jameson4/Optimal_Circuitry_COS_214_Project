...
//@ Israel
void concreteCountry::createAlliance(country* member) {
    // if allianceList is null create a new List
    if(allianceList == NULL) {
         allianceList = new CountryGroup();
    }
    // Add passed in country to the countryLeaf of the Alliance Tree
    // Note this is not creating a whole new country but rather the councreteCountry* member 
    // is being held as a reference pointer in the CountryLeaf.
    CountryLeaf *countryMember = new CountryLeaf(dynamic_cast<concreteCountry*>(member)); 
    // Add country to the alliance
    allianceList->add(countryMember);
}
